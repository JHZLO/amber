...
**자식 투두는 절대 조상 없이 읽지 마라 — 실제로 리포트를 틀리게 만든 실패다.**
자식 줄에는 `(상위: A › B)` 로 조상 경로가 함께 온다. 들여쓰기와 이 경로가 같은 말을 한다.
- `[x] 개발 적용  (상위: kafka 리밸런싱 알고리즘 변경 › GracefulKafkaModule 신설 및 배포)` 는
  "개발 적용"이라는 일이 아니라 **GracefulKafkaModule 을 개발 환경에 적용한 일**이다.
  제목에 조상의 주제를 담는다: `## GracefulKafkaModule 개발 환경 적용` (O) /
  `## 개발 환경 적용` (X — 무엇의 적용인지 사라진다).
- 자식 문구가 "개발 적용 · 운영 적용 · 1차 · 확인" 처럼 **그 자체로는 뜻이 없는 단계 이름**이면
  조상 없이는 항목이 될 수 없다. 조상 경로를 반드시 제목에 반영한다.
- **경로가 다른 항목을 한 항목으로 묶지 마라.** 상위가 다르면 다른 일이다. 근거(PR·세션)를
  고를 때도 그 경로의 주제와 맞는 것만 붙인다 — 옆 항목의 PR 을 끌어와 붙이면 없는 일을 만든다.
...
**줄글 문단은 인용(`>`)으로 감싼다.** 왼쪽에 얇은 세로 바가 붙어 '이건 배경 설명'임이
한눈에 보이고, 아래 작업 목록과 층이 갈린다. 문단이 여러 줄이면 **줄마다** `>` 를 붙인다.
(`>` 다음에 `[!NOTE]` 같은 대괄호 표식을 넣지 마라 — 그건 알림 블록 문법이라 색이 붙는다.)
...
- 줄글에 `>` 를 안 붙이거나 첫 줄에만 붙이기 ← 여러 줄이면 줄마다 붙인다
...
fn assemble_input(date: &str, todos_digest: &str, digests: &mut [SourceDigest]) -> String {
    ...
    let mut input = format!(
        "[리포트 대상 날짜]\n{date}\n\n         [투두 — 내가 오늘 적은 일. **이 리포트의 축이다.** `[x]` = 내가 완료 표시한 것.\n         들여쓰기 = 하위 항목이고, 줄 끝의 `(상위: A › B)` 가 그 줄의 조상 경로다]\n"
    );
