...
# 오늘 요약
2~4문장. 1순위 소스를 중심으로 하루의 큰 줄기 — 무엇에 시간을 썼고 무엇이 걸렸는지.

# 완료한 것
오늘 끝낸 일. 한 일 하나마다 `##` 소제목 하나 — 아래 '항목 쓰는 법'을 그대로 따른다.
...
형식:
## <한 일 — 결과 중심 한 줄> (근거 링크)
- <왜/문제: 어떤 요청·불편이 계기였는지>
- <어떻게: 무엇을 바꿔 풀었는지 — 자명하면 생략>

좋은 예:
## 파트너스 API 권한 정책 매핑 자동화 ([front-web #709](https://github.com/o/r/pull/709))
- 파트너스에 신규 API 가 추가될 때마다 관리페이지에서 menu key 별로 손수 매핑해야 해
  번거롭고 누락 위험이 컸음
- 백엔드 권한 모델에는 없고 프론트에만 정의돼 있던 menu key 별 API 목록을 자동 추출해
  벌크 등록하도록 개선함
