...
--
-- due_date 는 그대로 재사용하되(주 항목은 **그 주 월요일**), scope 로 성격을 가른다.
...
-- 날짜(월요일)라, 필터를 안 걸면 그 월요일의 목록·밀린 항목·캘린더 점에 그대로 섞인다.
-- 그래서 lib/todos.ts 의 listTodos / listMonthCounts / listOverdueOpen 은 전부
-- scope = 'day' 를 명시한다. 새 조회를 추가할 때도 반드시 scope 를 정하고 쓸 것.
--
-- 주는 월~일이다 (lib/date.ts mondayOf) — 미니 캘린더 표시(일~토)와 다른 건 의도이며,
-- 주간 리포트의 스프린트 규약과 같은 기준을 쓴다.
