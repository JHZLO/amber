...
- **Marks that touch get square corners.** `rx` rounds all four corners of a rect, so two abutting segments each pull their fill back at the shared edge. The seam shows a notch and the run reads as blocks of unequal height — the most common "why is this misaligned?" report. Any rect that shares an edge with another one — a timeline's stages, a stacked run, a lattice cell — uses `rx="0"`. Keep `rx="3"` for a mark that stands alone. Abutting segments must also share the same `y` and `height`; a run is one band, not a staircase.
