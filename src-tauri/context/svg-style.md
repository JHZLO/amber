...
- The one place a background color is allowed is an **opaque text plate**, and it is written as `style="fill: var(--surface, #ffffff)"` — never a literal. `--surface` is the app's canvas color and follows the theme (white on light, `#161618` on dark); the fallback only serves viewers without the app's stylesheet. A tint on top of a plate is still drawn with `currentColor` or a budget hue at low opacity.
...
- **Text plates are opaque.** Any box that carries text and sits over other geometry — a sequence-diagram note, a callout, a legend chip, a label on a lifeline or an axis — is drawn as a plate, not a tint: (1) `<rect rx="4" style="fill: var(--surface, #ffffff)"/>` sized from the text estimate plus 12 units of padding on each side, (2) the optional tint on top (`currentColor` at `0.04`, or the contrast hue at `0.12` when the plate is the highlight — over an opaque plate it reads as a solid tinted card), (3) the stroke, (4) the text. Because a plate hides what is under it, draw the lines it crosses **earlier in the document** and every plate later; a lifeline or grid line must never show through text. Where a plate would hide something that matters, break the line instead: end it 8 units above the text and resume 8 units below.
...
- **Notes.** A note over lanes i…j spans `x_i − L/2 + 8` to `x_j + L/2 − 8`, height 28 for one line or 44 for two, radius 4, and follows the text-plate recipe in §3: an opaque `var(--surface)` plate first, then `currentColor` at `0.04` (or the contrast hue at `0.12` for the one highlighted note), stroke `0.28`, text centered at 11.5. Lifelines are drawn before any note and pass under it — they never show through. A note occupies its own row. A note over a single lane is centered on that lane; if its text needs more than `L − 16`, widen it symmetrically around `x_i` (it may then cross the neighboring lifelines, which the plate hides) — never shift it off-center to avoid them.
